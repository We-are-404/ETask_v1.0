...
public class JwtInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		Claims body;
		try{
			body = JwtUtil.parseJwt(token, this.jwtProperties.getPrivateSecret());
		...
		if(differentMillis < this.jwtProperties.getRefreshSecond() * 1000 && differentMillis >= 0){
			String refreshJwt = JwtUtil.refreshJwt(body);
